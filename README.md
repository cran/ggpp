...
coordinates under a different name in the `data` object when displacing
them to a new position. This makes them compatible with `geom_text_s()`,
...
with ‘ggplot2’. Starting from version 0.9.2 geometries
`geom_text_repel()` and `geom_label_repel()` from package ‘ggrepel’ are
fully compatible with this new naming convention.
