...
we prepend `NA`.) However, scale limits are not recomputed and modifying
them would exclude observations. Consequently, in this case we need to
manualy set the limit in `coord_cartesian()`.
...
both `x` and `y` are passed as arguments, or towards opposite sides
of a boundary line if only one of `x` or `y` is passed an argument.
By default, the "center" is the centroid computed using `mean()`, but other
functions or numeric values can be passed to override it. 
