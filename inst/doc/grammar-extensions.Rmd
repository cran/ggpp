...
We load all the packages used in the examples, if installed, and set flags to
skip examples if they are not available.
...
(2016) and several other books. Chapter 7 in Aphalo (2020) not only describes
the grammar used by 'ggplot2' but also several extensions to it, including 
those provided by packages 'ggpp' and 'ggpmisc'.
...
elements from the data source and faceting, but not from the scales
...
Package 'ggpp' adds support for various plot annotations and
...
| `ggrepel::geom_text_repel()`   |                    | `character` vector          |
| `ggrepel::geom_label_repel()`  |                    | `character` vector          |
| `ggplot2::geom_label()`        | `geom_label_npc()` | `character` vector          |
| `geom_text_linked()`           | _coming soon_      | `character` vector          |  
...
These new position functions are compatible with 'ggrepel' (>= 0.9.2) and
similarly to `position_nudge_repel()` support drawing segments or arrows
connecting the original position to the nudged position.
...
Aphalo, Pedro J. (2020) _Learn R: As a Language._ The R Series. Boca Raton and
London: Chapman and Hall/CRC Press. ISBN: 978-0-367-18253-3. 350 pp.
...
Some of the examples below are evaluated only if packages 'ggrepel',
and 'magick' are available.
...
`time` to `numeric` is also possible (shown in the second example below).
...
The class of variable `time`, mapped to the _x_ aesthetic, affects the scale
used by default as well as the formatting of values when converted to character
strings or printed. Here we force its conversion to `numeric`.
...
Three of the geometries described below allow the addition of plot layers
...
grouping, multiple insets per panel, faceting with different tables per panel,
and different number of insets in each panel, i.e., individual tables added to a
...
geometries are not meant to be used to display data, but instead they make it
possible to add annotations to plots consistently across data sets even when
using scales with varying limits.
...
always added at their native sizes, which can be altered by changing the `size`
of the text in them.
...
constants are mapped to the `colour`, `fill`, `size`, or `family`, aesthetics
they override the corresponding default theme settings. The display of
`rownames` and `colnames` can be enabled or disable through parameter
`table.rownames` and `table.colnames` and the horizontal justification of text
in the core of the table through parameter `table.hjust`.
...
plain text in case of parsing errors, on a cell by cell basis (see `tableGrob()`
in package 'gridExtra' for details, as this function is used to build the
table). Here we plot the MPG for city traffic and we can see that the plotting
area expands to include the coordinates at which the table is anchored.
Justification is by default set to `"inward"` which ensures that the table is
...
superimposed on different countries or regions. The size of the table inset is
given by the _size_ aesthetic, so like for `geom_text()` it is independent of
the `ggplot2::theme()` and controlled by the `table.theme` directly or
indirectly through the _size_ aesthetic.
...
be superimposed on different countries or regions. The size of the insets is
controlled by the `vp.width` and `vp.height` aesthetics as a fraction of the
main plot's plotting region. Consequently, the insets are scaled together with
the main plot. A possible unintuitive but useful feature, is that the theme is
linked to each plot.
...
The `geom_grob()` *geometry* plots grobs (graphical objects as created with
'grid'), nested in a tibble passed as data argument, using *aesthetics* `x` and
`y` for positioning, and `label` for the Grob object. While `geom_table()` and
`geom_plot()` take as values mapped to the `label` aesthetics tibbles or data
frames, and ggplots, respectively, and convert them into Grobs, `geom_grob()`
expects Grobs ready to be rendered. This means that any Grob created using
'grid' or its extenssions can be added as a data label to a ggplot.
...
annotations, although one could use it for infographics with multiple copies of
each grob, this would go against the grammar of graphics. In this implementation
grobs cannot be mapped to an aesthetic through a scale.
...
insets is controlled by the `vp.width` and `vp.height` aesthetics as a fraction
of the main plot's plotting region. Consequently, the insets are scaled together
with the main plot.
...
With package 'ggrepel' we have become used to easily add text and labels linked
to the labelled observations or points by a line segment or arrow. However,
sometimes a plot has few text labels and repulsion is unnecessary. In such
cases nudging or dodging may be enough, or even preferable so as to achieve
consistency.

We can achieve this with `geom_text_linked()` together with a suitable position
function that retains the original _x_ and _y_ coordinates of the point. We
currently provide three different nudge functions described in
a later section. One of them is used by default providing the same default
behaviour as `geom_text()` from 'ggplot2' but drawing linking segments if
nudging is set to a value different to the default of zero.

**Please, be aware that this is a preliminary implementation, whose user interface
and behavior are very likely to change in coming months.**
...
locations of plot elements. In 'ggplot2' the *x* and *y* aesthetics are directly
...
implemented as a 'grid' viewport and support for `"npc"` coordinates is
relatively easy to implement.

To support `"npc"` positions we have implemented *scales* for two new (pseudo)
*aesthetics*, `npcx` and `npcy`. These are very simple continuous scales which
...
`mean_se()` to both `x` and `y` by group. If the value mapped to
an aesthetics across rows within each group is unique this value is copied to the returned `data`.
...
This statistic applies functions to `x` and `y` data. The function(s) supplied as argument are expected to return a numeric vector. Both functions should return vectors of the same length. When possible it is
usually better to apply functions through mappings using `aes()`
when they are independent of grouping, or to use a transformation for the
scale. However, when grouping is important, these statistics make it possible
to avoid pre-computation of the data for a layer. 

Here we plot the difference in trunck circumference between dates for each tree.
(As the vector returned by `diff()` is one element shorter than its input, we
delete the first value of `x` in `.fun.x`.).
...
Function `position_nudge()` from package 'ggplot2' simply applies the nudge, or
*x* and *y* shift based directly on the values passed to its parameters `x` and
`y`. A new variation on simple nudge is provided by function
`position_nudge_to()`, which accepts the desired nudged coordinates directly.

Two new position functions supporting advanced nudging are also provided. While
`position_nudge()` implements shifts along $x$ and $y$ axes that are the same
for every observation, the new functions compute the direction of the nudging
specifically for each observation. One can think of this approach as nudging
away from a focal point, a line or a curve. It is deterministic, while repulsion
is aleatory. Advanced nudging works very well, but only for some patterns of
observations and may require manual adjustment of positions, repulsion is more
generally applicable.

All the new position functions save the original coordinates to allow linking
the nudged objects, usually text labels, to be linked to the original
position. Linking is supported by `geom_text_linked()` from this package and
by `geom_text_repel()` and `geom_label_repel()` from 'ggrepel' 
(in the future >= 0.9.2). They can be also used with all other geoms as the
nudged coordinates are returned in the same way as by other 'ggplot2'
position functions, but other geoms will ignore the original positions stored
in `data`.

### position_nudge_keep()

`position_nudge_keep()`, and `position_nudge_center()` With the default 
`direction = "none"`, behave as `ggplot2::position_nudge()` but save the 
original coordinates as `ggrepel::position_nudge_repel()` does.
...
Here we keep the default justification, with text centred on its location.
...
```
When using `geom_text_linked()` we need to change the default justification.
...
As this is also the default position function of this geometry, the code below
is equivalent.
...
Function `position_nudge_to()` differs from `ggplot2::position_nudge()` in that
the coordinates of the new positions are supplied directly rather than as a
displacement.
...
boundary line if only one of `x` or `y` is passed an argument. By default, the
"center" is the centroid computed using `mean()`, but other functions or numeric
values can be passed to override it.
...
When using one of the directions `"split"` or `"radial"` by default a centroid
computed with `mean()` is used as focal point. If the direction is `"split"`
then only the sign of `x` and/or `y` are reversed to the left and below the
focal point, respectively.
...
By default `x` and `y` take a value of zero.
...
We can override the default center coordinates by passing numeric values to
`center_x` and/or `center_y`.
...
We can override the default center coordinates computed with `mean()` by passing
as argument a different function.
...
Direction `"radial"` applies nudge radially away from the focal point.
...
Function `position_nudge_line()` will work without change irrespective
