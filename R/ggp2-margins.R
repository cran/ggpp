...
#' @param margin Margins around the text. See [ggplot2::margin()] for more
#'   details.
...
titleGrob <- function(label, x, y, hjust, vjust, angle = 0, gp = gpar(),
  ...
  width  <- grid::unit(1, "grobwidth",  grob) + x_descent
  height <- grid::unit(1, "grobheight", grob) + y_descent
  ...
  if (margin_x) {
    new_width <- grid::unit.c(margin[4], width, margin[2])
    new_x <- x - margin[2] * just$hjust + margin[4] * (1 - just$hjust)
  }

  # Calculate new y/height
  if (margin_y) {
    new_height <- grid::unit.c(margin[1], height, margin[3])
  ...
  if (xor(margin_x, margin_y)) {
    new_width  <- new_width  %||% grid::unit(1, "null")
    new_height <- new_height %||% grid::unit(1, "null")
  }
  ...
  if (isTRUE(debug)) {
    children <- grid::gList(
      grid::rectGrob(
        ...
        gp = grid::gpar(fill = "cornsilk", col = NA)
      ),
      grid::pointsGrob(x, y, pch = 20, gp = grid::gpar(col = "gold")),
      grob
    )
  } else {
...
justify_grobs <- function(grobs, x = NULL, y = NULL, hjust = 0.5, vjust = 0.5,
  ...
  if (!inherits(grobs, "grob")) {
    if (is.list(grobs)) {
      ...
      stop("Expected an individual 'grob' or list of 'grob' objects, but 'grobs' is a '", class(grobs)[1], "'")
    }
  }
  ...
  x <- x %||% grid::unit(just$hjust, "npc")
  y <- y %||% grid::unit(just$vjust, "npc")


  if (isTRUE(debug)) {
    children <- grid::gList(
      grid::rectGrob(gp = grid::gpar(fill = "lightcyan", col = NA)),
      grobs
    )
  }
  else {
  ...
  if (isTRUE(debug)) {
    ...
    grid::grobTree(
      result_grob,
      grid::pointsGrob(x, y, pch = 20, gp = grid::gpar(col = "mediumturquoise"))
    )
  } else {
    result_grob
  }
