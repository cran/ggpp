...
**We will use package 'ggrepel' in some examples. Only versions newer than 0.9.1
are compatible with the position functions from 'ggpp'. The current CRAN version
is 0.9.1. Consequently, 'ggrepel' will need to be installed from GitHub until
version 0.9.2 is submitted to CRAN.
...
currently in package 'ggpp'.

Before showing how this works, we load the package and set an uncluttered theme
as default.
...
together with a suitable position function that retains the original *x* and *y*
coordinates of the point. We provide several different position functions
described in a later section, all of which retain the initial position in
`data$x_orig` and `data_$y_orig`. Function `position_nudge_keep()` is used by
default providing the same default behaviour as `geom_text()` and `geom_label()`
from 'ggplot2'. However, when nudging or a position
function that keeps the original position is used, by default linking segments are drawn
between the original and nudged or displaced positions. Passing `add.segments = FALSE` disables the drawing of segments. The equivalent for nudged points
connected by segments to their original positions is `geom_point_s()`.
...
The labels can be rotated as long as the geometry used supports this. In the
next few examples nudging is not enough to avoid overlaps, so we use repulsive
geometries from package 'ggrepel'. **The current CRAN release of 'ggrepel' (==
0.9.1) does not work correctly with the position functions from 'ggpp'. The
development version of 'ggrepel' that can be installed from CRAN and released in
the future as version (== 0.9.2) works correctly. Incompatible versions of
'ggrepel' do not crash but the segments are not correctly drawn because of the
different naming conventions previously used in the 'ggpp' and 'ggrepel' to
...
data. We however shift the _x_ center point manually to avoid an overlap.
...
position functions available in 'ggplot2'.
...
for observations and text to be the same.**
...
original and displaced positions as saved by these position functions.
...
or from the original positions. In this example, nudge away from the jittered
positions with `nudge.from = "jittered"` nad the direction of the nudge is on
...
In this second example, the nudging is away from the original _x_-position and the
